...
class MySceneGraph {
    ...
    */
    ...
    parseComponents(componentsNode) {
        ...
        var comp = new MyComponent(idComponent);
        ...
        if ((index = nodeNames.indexOf("texture")) == -1)
            return "tag <texture> missing inside primitives";
        else {
            var id2 = this.reader.getString(children[index], 'id');
            if (this.textures.hasOwnProperty(id2) || id2=="none" || id2=="inherit"){
                if(id2!="none"){//if id is "none" there's no need to check length_s and length_t
                    if(this.reader.hasAttribute(children[index], 'length_s')) //checking if length_s exists
                        length_s = this.reader.getFloat(children[index], 'length_s');
                    else if(id2 != "inherit") {
                        onXMLMinorError("length_s missing, default value = 1");
                        length_s=1;
                    }
                    if(this.reader.hasAttribute(children[index], 'length_t')) //checking if length_t exists
                        length_t = this.reader.getFloat(children[index], 'length_t');
                    else if(id2 != "inherit") {
                        onXMLMinorError("length_t missing, default value = 1");
                        length_t=1;
                    }
                }
                var texture = [id2, length_s, length_t];
     ...
     */
    displayScene( idNode, idMaterialFather, idTextureFather, lengthSFather, lengthTFather) {
        ...
        if(comp.texture[0] != "none" && comp.texture[0] != "inherit") //binding texture
        {
            lengthSSon = comp.texture[1];
            lengthTSon = comp.texture[2];
            for(var c = 0; c < comp.primitives.length; c++)
            {
                this.primitives[comp.primitives[c]].alterLengthST(comp.texture[1],comp.texture[2]);
            }
            this.textures[comp.texture[0]].bind(1);
            this.textures[comp.texture[0]].bind();
            textureSon = comp.texture[0];
        } else if(comp.texture[0] == "none" && idTextureFather != "none") //taking out the texture
            this.textures[idTextureFather].unbind();
        else if(comp.texture[0] == "inherit") //altering length_s and length_t accordingly
        {   
            if(!comp.texture[1])
                comp.texture[1]=lengthSSon;
            else
                lengthSSon = comp.texture[1];
            if(!comp.texture[2])
                comp.texture[2]=lengthTSon;
            else
                lengthTSon = comp.texture[2];
            for(var c = 0; c < comp.primitives.length; c++)
            {
